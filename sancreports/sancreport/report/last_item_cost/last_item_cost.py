...
def execute(filters=None):
    data = frappe.db.sql("""
        SELECT
            i.item_code,
            i.item_name,
            sle.posting_date AS latest_posting_date,
        ...
        LEFT JOIN `tabLanded Cost Item` lci
            ...
            AND lci.parent = (
                ...
                JOIN `tabLanded Cost Item` lci2 ON lci2.parent = lcv2.name
    ...
    for row in data:
        if row.latest_posting_date:
            row.latest_posting_date = row.latest_posting_date.strftime("%d-%m-%Y")

    columns = [
        {"label": "Item Code", "fieldname": "item_code", "fieldtype": "Data", "width": 160},
        {"label": "Item Name", "fieldname": "item_name", "fieldtype": "Data", "width": 260},
        {"label": "Latest Incoming Date", "fieldname": "latest_posting_date", "fieldtype": "Data", "width": 150},
